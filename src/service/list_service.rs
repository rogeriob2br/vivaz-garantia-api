...
pub fn rem_list(settings: &RedisConfig, req: RepoList) -> Result<(), RedisError> {
    let client = RepoClient::new(settings);
    let repo = RepoClient{
        db: client.unwrap(),

    };
    let resp = RepoList::rem(req,repo);
    resp
...
pub fn map_repo_list_full( k:String, v: Vec<String>) ->RepoList{
    RepoList{
        value: v,
        key: k,
        ttl: 0
